...
export const useItems = () => useQuery(['items'], () => send('/api/items'));

export const useItem = ({ itemId }) =>
  useQuery(['item', itemId], () => send(`/api/items/${itemId}`), { enabled: !!itemId });
...
export const postItem = ({ name, description, brandId, typeId, expirationDate, imageUrl }) =>
  send('/api/items', 'post', { name, description, brandId, typeId, expirationDate, imageUrl });
